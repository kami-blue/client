...
class Tracers : Module() {
    ...
    private val neutral = register(Settings.booleanBuilder("NeutralMobs").withValue(true).withVisibility { mobs.value }.build())
    private val hostile = register(Settings.booleanBuilder("HostileMobs").withValue(true).withVisibility { mobs.value }.build())
    private val range = register(Settings.d("Range", 200.0))
    private val renderInvis = register(Settings.b("Invisible", false))
    private val customColours = register(Settings.booleanBuilder("CustomColours").withValue(true).build())

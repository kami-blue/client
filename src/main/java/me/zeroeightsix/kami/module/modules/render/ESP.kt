...
object ESP : Module(
    ...
    private val page by setting(getTranslationKey("Page"), Page.ENTITY_TYPE)

    /* Entity type settings */
    private val all by setting(getTranslationKey("AllEntity"), false, { page == Page.ENTITY_TYPE })
    private val experience by setting(getTranslationKey("Experience"), false, { page == Page.ENTITY_TYPE && !all })
    private val arrows by setting(getTranslationKey("Arrows"), false, { page == Page.ENTITY_TYPE && !all })
    private val throwable by setting(getTranslationKey("Throwable"), false, { page == Page.ENTITY_TYPE && !all })
    private val items by setting(getTranslationKey("Items"), true, { page == Page.ENTITY_TYPE && !all })
    private val players by setting(getTranslationKey("Players"), true, { page == Page.ENTITY_TYPE && !all })
    private val friends by setting(getTranslationKey("Friends"), false, { page == Page.ENTITY_TYPE && !all && players })
    private val sleeping by setting(getTranslationKey("Sleeping"), false, { page == Page.ENTITY_TYPE && !all && players })
    private val mobs by setting(getTranslationKey("Mobs"), true, { page == Page.ENTITY_TYPE && !all })
    private val passive by setting(getTranslationKey("PassiveMobs"), false, { page == Page.ENTITY_TYPE && !all && mobs })
    private val neutral by setting(getTranslationKey("NeutralMobs"), true, { page == Page.ENTITY_TYPE && !all && mobs })
    private val hostile by setting(getTranslationKey("HostileMobs"), true, { page == Page.ENTITY_TYPE && !all && mobs })
    private val invisible by setting(getTranslationKey("Invisible"), true, { page == Page.ENTITY_TYPE && !all })
    private val range by setting(getTranslationKey("Range"), 32.0f, 8.0f..64.0f, 0.5f, { page == Page.ENTITY_TYPE })

    /* Rendering settings */
    private val mode = setting(getTranslationKey("Mode"), ESPMode.SHADER, { page == Page.RENDERING })
    private val hideOriginal by setting(getTranslationKey("HideOriginal"), false, { page == Page.RENDERING && mode.value == ESPMode.SHADER })
    private val filled by setting(getTranslationKey("Filled"), false, { page == Page.RENDERING && (mode.value == ESPMode.BOX || mode.value == ESPMode.SHADER) })
    private val outline by setting(getTranslationKey("Outline"), true, { page == Page.RENDERING && (mode.value == ESPMode.BOX || mode.value == ESPMode.SHADER) })
    private val r by setting(getTranslationKey("Red"), 155, 0..255, 1, { page == Page.RENDERING && (mode.value == ESPMode.BOX || mode.value == ESPMode.SHADER) })
    private val g by setting(getTranslationKey("Green"), 144, 0..255, 1, { page == Page.RENDERING && (mode.value == ESPMode.BOX || mode.value == ESPMode.SHADER) })
    private val b by setting(getTranslationKey("Blue"), 255, 0..255, 1, { page == Page.RENDERING && (mode.value == ESPMode.BOX || mode.value == ESPMode.SHADER) })
    private val aFilled by setting(getTranslationKey("FilledAlpha"), 63, 0..255, 1, { page == Page.RENDERING && (mode.value == ESPMode.BOX || mode.value == ESPMode.SHADER) })
    private val aOutline by setting(getTranslationKey("OutlineAlpha"), 255, 0..255, 1, { page == Page.RENDERING && (mode.value == ESPMode.BOX || mode.value == ESPMode.SHADER) })
    private val blurRadius by setting(getTranslationKey("BlurRadius"), 0f, 0f..16f, 0.5f, { page == Page.RENDERING && mode.value == ESPMode.SHADER })
    private val width by setting(getTranslationKey("Width"), 2f, 1f..8f, 0.25f, { page == Page.RENDERING })

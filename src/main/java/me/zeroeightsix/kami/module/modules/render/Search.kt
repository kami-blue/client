...
object Search : Module(
    ...
    private val updateDelay by setting(getTranslationKey("UpdateDelay"), 1000, 500..3000, 50)
    private val range by setting(getTranslationKey("SearchRange"), 128, 0..256, 8)
    private val maximumBlocks by setting(getTranslationKey("MaximumBlocks"), 256, 16..4096, 128)
    private val filled by setting(getTranslationKey("Filled"), true)
    private val outline by setting(getTranslationKey("Outline"), true)
    private val tracer by setting(getTranslationKey("Tracer"), true)
    private val customColors by setting(getTranslationKey("CustomColors"), false)
    private val r by setting(getTranslationKey("Red"), 155, 0..255, 1, { customColors })
    private val g by setting(getTranslationKey("Green"), 144, 0..255, 1, { customColors })
    private val b by setting(getTranslationKey("Blue"), 255, 0..255, 1, { customColors })
    private val aFilled by setting(getTranslationKey("FilledAlpha"), 31, 0..255, 1, { filled })
    private val aOutline by setting(getTranslationKey("OutlineAlpha"), 127, 0..255, 1, { outline })
    private val aTracer by setting(getTranslationKey("TracerAlpha"), 200, 0..255, 1, { tracer })
    private val thickness by setting(getTranslationKey("LineThickness"), 2.0f, 0.25f..5.0f, 0.25f)

    var overrideWarning by setting(getTranslationKey("OverrideWarning"), false, { false })

...
object BreakingESP : Module(
    ...
    private val espSelf = setting(getTranslationKey("ESPSelf"), true)
    private val warnSelf = setting(getTranslationKey("WarnSelf"), false)
    private val obsidianOnly = setting(getTranslationKey("ObsidianOnly"), false)
    private val warning = setting(getTranslationKey("Warn"), false)
    private val warningProgress = setting(getTranslationKey("WarnProgress"), 4, 0..10, 1)
    private val chatWarn = setting(getTranslationKey("ChatWarning"), false)
    private val screenWarn = setting(getTranslationKey("HUDWarning"), true)
    private val soundWarn = setting(getTranslationKey("SoundWarning"), false)
    private val range = setting(getTranslationKey("Range"), 16.0f, 2.0f..32.0f, 2.0f)
    private val filled = setting(getTranslationKey("Filled"), true)
    private val outline = setting(getTranslationKey("Outline"), true)
    private val tracer = setting(getTranslationKey("Tracer"), false)
    ...
    private val aTracer = setting(getTranslationKey("TracerAlpha"), 255, 0..255, 1, { outline.value })

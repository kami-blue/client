...
object ChatEncryption : Module() {
    ...
    private val mode by setting("EncryptMode", EncryptionMode.VIGNERE)
    private val keyA by setting("KeyA", 3, 0..26, 1,  { mode != EncryptionMode.VIGNERE })
    private val keyB by setting("KeyB", 10, 0..26, 1,  { mode != EncryptionMode.VIGNERE })
    private val vignereKey by setting("VignereKey", "DefaultKey")
    private val useDelimiter by setting("UseDelimiter", false)
    val delimiter by setting("Delimiter", "Unchanged")
    ...
    private val delimiterValue: String?
        get() {
            if (delimiter == "Unchanged") {
                MessageSendHelper.sendErrorMessage("$chatName Please change the delimiter in ClickGUI")
                disable()
                return null
            }
            return delimiter
    ...
    private val vignereKeyValue: String?
        get() {
            if (vignereKey == "DefaultKey") {
                MessageSendHelper.sendErrorMessage("$chatName Please change the key in ClickGUI, then make sure to re-enable $name!")
                disable()
                return null
            }
            return vignereKey
    ...
    init {
        ...
        listener<PacketEvent.Receive> {
            ...
            if (!self && isOwn(s)) return@listener
    ...
    private fun getKey(): Int {
        return sqrt((keyA * keyB).toDouble()).toInt()
    }

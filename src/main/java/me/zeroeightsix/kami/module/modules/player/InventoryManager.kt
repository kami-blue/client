...
class InventoryManager : Module() {
    ...
    private val refillThreshold = register(Settings.integerBuilder("RefillThreshold").withValue(16).withRange(1, 48).withVisibility { autoRefill.value }.build())
    ...
    val pauseMovement: Setting<Boolean> = register(Settings.b("PauseMovement", true))
    private val delay = register(Settings.integerBuilder("DelayTicks").withValue(1).withRange(1, 10).build())

    enum class State {
        IDLE, SAVING_ITEM, REFILLING_BUILDING, REFILLING
    }
    ...
    override fun onUpdate() {
        if (mc.player.isSpectator || mc.currentScreen != null) return

        if (tickCount < delay.value) {
            tickCount++
            return
        } else tickCount = 0

        setState()
        sendChatMessage(currentState.toString())
        when (currentState) {
        ...
        if (currentState != State.IDLE) mc.playerController.syncCurrentPlayItem()
    }
    ...
    private fun setState() {
        ...
        if (currentState == State.IDLE) inProcess = false
    ...
    private fun refillBuildingCheck(): Boolean {
        if (!autoRefill.value || !buildingMode.value || buildingBlockID.value.toInt() == 0 || getSlotsNoHotbar(buildingBlockID.value) == null) return false

        val itemCount = countItem(0, 8, buildingBlockID.value.toInt())
        sendChatMessage(itemCount.toString())
        return itemCount <= refillThreshold.value || (findRefillableSlotBuilding() != null && inProcess)
    }

    private fun refillCheck(): Boolean {
        if (!autoRefill.value) return false

        return findRefillableSlot() != null || (findRefillableSlot() != null && inProcess)
    }
    /* End of state checks */
    ...
    private fun refillBuilding() {
        inProcess = findRefillableSlotBuilding() != null

        val slots = getSlotsFullInvNoHotbar(buildingBlockID.value)
        quickMoveSlot(slots?.get(0) ?: return)
    }

    private fun refill() {
        inProcess = findRefillableSlot() != null

...
class AntiHunger : Module() {
    private val cancelMovementState = register(Settings.b("Cancel Movement State", true))
    ...
    var packetListener = Listener(EventHook { event: PacketEvent.Send ->
        ...
        if (cancelMovementState.value && event.packet is CPacketEntityAction) {
            val packet = event.packet as CPacketEntityAction
        ...
        if (event.packet is CPacketPlayer) {
            ...
            (event.packet as CPacketPlayer).onGround = (mc.player.fallDistance <= 0 || mc.playerController.isHittingBlock) && mc.player.isElytraFlying
        }
    })
}
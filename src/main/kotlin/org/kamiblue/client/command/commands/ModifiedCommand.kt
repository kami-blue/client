        ...
        module("module") {
            execute("List modified settings") {
                ...
                for (setting in it.value.settingList) {
                    ...
                    component.style.clickEvent = ClickEvent(ClickEvent.Action.RUN_COMMAND, "${CommandConfig.prefix.value}set ${it.value.name} ${setting.name.replace(" ", "")} ${setting.defaultValue}")
